...
export const CaseStudyLayout = ({ article }: CaseStudyLayoutProps) => {
  ...
  return (
    <div className="case-study w-full bg-[rgb(249,250,251)] font-cs-sans text-zinc-900 dark:bg-[#1f1f1f] dark:text-zinc-100">
      ...
      <section className="mx-auto w-full max-w-[1080px] px-6 pt-14 sm:px-12">
        {/* chips */}
        {chips.length > 0 && (
          <div className="mb-8 flex flex-wrap gap-2">
            {chips.map(chip => (
              <span
                key={chip.id}
                className="rounded-full border border-black/[0.13] px-3.5 py-1.5 text-sm tracking-[0.01em] text-zinc-600 dark:border-white/15 dark:text-zinc-300"
              >
                {chip.name}
              </span>
            ))}
        ...
        <div className="mb-12 grid grid-cols-1 items-start gap-12 lg:grid-cols-[1fr_320px]">
          <div>
            <h1
              className="font-cs-serif text-4xl font-normal leading-[1.1] tracking-[-0.02em] text-zinc-900 dark:text-zinc-100 md:text-5xl lg:text-6xl"
      ...
      <div className="mx-auto w-full max-w-[1080px] border-t border-black/[0.08] dark:border-white/10">
        <div className="grid grid-cols-1 items-start lg:grid-cols-[220px_1fr]">
          {/* sticky TOC */}
          {tocItems.length > 0 && (
            <aside className="hidden lg:sticky lg:top-14 lg:flex lg:max-h-[calc(100vh-56px)] lg:flex-col lg:overflow-y-auto lg:border-r lg:border-black/[0.08] lg:px-7 lg:py-12 dark:lg:border-white/10">
